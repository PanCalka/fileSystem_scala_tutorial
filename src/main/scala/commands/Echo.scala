...
class Echo(tail: Array[String]) extends Command {
  ...
  def doEcho(state: State, content: String, filename: String, append: Boolean): State = {
    if(filename.contains(Directory.SEPARATOR))
      state.setMessage("Echo: files must not contain separators")
    else {
  ...
  override def apply(state: State): State = {
    if(tail.isEmpty) state
    else if(tail.length ==1) state.setMessage(tail(0))
    else {
      val operator = tail(tail.length -2)
      val filename = tail(tail.length -1)
      val content  = createContent(tail, tail.length-2)
      ...
      if(">>".equals(operator))
        ...
        state.setMessage(createContent(tail, tail.length))
    }
  }
}
